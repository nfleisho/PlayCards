...
function handDisplay(hand) {
    var dispstr = ["", ""];
    for (var jsuit = 0; jsuit < suitlist.length; jsuit++) {
        dispstr[0] += "<br><strong>&" + suitlist[jsuit] + ";: </strong>";
        dispstr[1] += "<br>";
        var suitsub = hand.filter(function (crds) {
            return crds.suit == suitlist[jsuit]
        });
...
function dealHand() {
    ...
    for (var item in table) {
        ...
        var imdisp = handDisplay(table[item])[1];
        var totdisp = "<span class='tblpos'><b>" + players[index] + "</b></span>" + handdisp;
        var totimdisp = "<b>" + players[index] + "</b>" + imdisp;
        var divloc = "div" + index;
        var divimloc = "imdiv" + index;
        $("#" + divloc).prepend(totdisp);
        $("#" + divimloc).prepend(totimdisp);
        $(".imrw").hide();
        index += 1;
    }
}
